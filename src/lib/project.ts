...
/**
 * Get project navigation (next/previous) based on published projects
 */
export function getProjectNavigation(currentSlug: string): {
  previous: { title: string; slug: string } | null;
  next: { title: string; slug: string } | null;
} {
  const publishedProjects = getPublishedProjectCaseStudies();
  ...
  return {
    previous: previousProject
      ? {
          title: previousProject.frontmatter.title,
